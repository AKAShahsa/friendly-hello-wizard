// queueSheet.tsx file (Original version provided by user)
...
const QueueSheet: React.FC<QueueSheetProps> = ({
  ...
  const handlePlayTrack = (track: Track) => {
    ...
    // Note: There was a formatting issue here in the original paste, fixed closing brace placement
  };
            ...
            {queue.length > 0 ? (
              <div className="space-y-3">
                {queue.map(track => (
                  <div
                    key={track.id}
                    // Note: Original paste had a formatting issue here, fixed className string
